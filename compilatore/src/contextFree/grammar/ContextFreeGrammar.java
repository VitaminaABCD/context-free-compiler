...
public class ContextFreeGrammar implements IGrammar {
	 ...
	 * essa per l'appunto avr� la stessa dimensione di V. un elemento si dice
	 * nullo se ogni componente dell'espressione della produzione sar� nulla
	 * facciamo un ciclo while che si fermer� quando sono stati controllati
	 * tutti i nulli e contemporaneamente nn sono state fatte modifiche quindi
	 * il nostro flag sar� falso
	 */
	public void nullo() {
		...
		while (flag) {
			...
			// itero per ogni produzioni per vedere se � nulla
			while (posiz.hasNext()) {
				...
				while (termine.hasNext()) {
					...
					// se anche un solo simbolo dell'espressione � un elemento
					// terminale tutta l'espressione nn potr� essere nulla
					...
					// Se il simbolo � un non terminale devo vedere se � nullo
				...
				// Non Terminale � impostato a false allora
	 ...
	 * modifica fatta a un first pu� influire su tutti gli altri quinidi creiamo
	 * un flag che ci indicher� se sono state fatte modifiche e il nostro ciclo
	 * si fermer� quando esso sar� settato a false e restera tale per la
		...
		while (flag) {
			...
			while (posiz.hasNext()) {
				...
				while (termine.hasNext()) {
					...
					// Se il simbolo � un terminale,lo aggiungo a firstEspr
					...
					// Se il simbolo � un nt...
						...
						// Se il non terminale non � nullable ho finito di
				...
				// un Set verranno aggiunti solo i simboli che non sono gi�
				// presenti
				first[V.indexOf(ass)].addAll(firstEspr);
				// se il numero di elementi in first(ass) � cambiata devo
				// rieseguire tutto xk� potrebbe influire su tutti gli altri
	 ...
	 * una struttura di tipo Set per evitare ripetizioni come prima ogni
	 * modifica fatta a un follow pu� influire su tutti gli altri quinidi
	 * creiamo un flag che ci indicher� se sono state fatte modifiche e il
	 * nostro ciclo si fermer� quando esso sar� setto a false e restera tale per
			...
			while (posizione.hasNext()) {
				...
				while (!espr.isEmpty()) {
					...
					if (V.contains(termine)) {
						// se � un NON Terminale allora calcolo i follow per
						// esso
						...
						while (elemen.hasNext()) {
							simbolo = elemen.next();
							// Se Simbolo � un terminale lo aggiungo ai
							...
							// Se il Simbolo � un nt, aggiungo i first(simbolo)
							// ai follow(termine), controllo se Simbolo � nullo
							// e in caso positivo esco perch� ho finito
							else if (V.contains(simbolo)) {
						...
						// se tuttinull � vero vuol dire che il tutti gli
						...
						// Se il follow � cambiato, devo ricalcolare tutti i
